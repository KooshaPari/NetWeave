<script lang="ts">
    ...
    function createIntersection(fabric: any, intersectionType: IntersectionType, position: { x: number, y: number }) {
    ...
    function createEntryPoint(fabric: any, config: IntersectionType, angle: number) {
        ...
        return new fabric.Line([x, y, x + length * Math.cos(angle * Math.PI / 180), 
