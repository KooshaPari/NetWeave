...
// src/lib/components/OutputCanvas.svelte
<script lang="ts">
    import { onMount } from 'svelte';
...
<script lang="ts">
    ...
    const tools = [
        ...
        { id: 'trafficLight', label: 'Traffic Light', icon: '🚦' }
    ];
</script>
...
<div class="flex gap-2 mb-4">
    {#each tools as tool}
        <button
            class="px-4 py-2 rounded flex items-center gap-2 {selectedTool === tool.id ? 'bg-blue-500 text-white' : 'bg-gray-200'}"
            on:click={() => dispatch('toolSelect', tool.id)}
        >
            <span>{tool.icon}</span>
