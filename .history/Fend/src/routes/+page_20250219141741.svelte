<script lang="ts">
	...
	function renderPreSimulation() {
		...
		simulationView.projects[0].layers[0].children.forEach((path: paper.Path) => {
			if (path.strokeColor) {
				// Add shadow effect
	...
	function renderSimulation() {
		...
		paper.view.onFrame = (event: paper.Event) => {
			vehicles.forEach((vehicle: paper.Path.Circle) => {
	...
	function createVehicles() {
		const vehicles = [];
		const roads = simulationView.projects[0].layers[0].children;

		roads.forEach((road: paper.Path) => {
			if (road.strokeColor) {
				// Create vehicle
	...
	<div class="container mx-auto">
		<div class="flex gap-4">
			...
			<div class="flex-1 rounded-lg bg-white p-4 shadow-lg">
				...
				<canvas bind:this={inputCanvas} width={600} height={400} class="rounded border" />
			</div>
			...
			<div class="flex-1 rounded-lg bg-white p-4 shadow-lg">
				...
				<canvas bind:this={outputCanvas} width={600} height={400} class="rounded border" />
			</div>
		</div>
	</div>
</main>
