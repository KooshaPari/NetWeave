<script lang="ts">
	...
	import RoadEditorToolbar from './RoadEditorToolbar.svelte';
	...
	let drawingState = {
		currentPath: null as paper.Path | null,
		pathPoints: [] as paper.Point[],
		lastDrawTime: 0,
		isDrawing: false,
		isDragging: false
	};

	// Add reactive statements for tool changes
	$: if (currentTool && roadNetwork) {
	...
	// Optimized road tool setup
	function setupRoadTool() {
		const tool = new paper.Tool();

		tool.onMouseDown = (event: paper.ToolEvent) => {
			if (drawingState.isDragging) return;
			...
			if (!selectedRoadType.isOneWay && selectedRoadType.lanes > 1) {
				addLaneMarkings(drawingState.currentPath);
			}
		};

		tool.onMouseDrag = throttle((event: paper.ToolEvent) => {
		...
		tool.onMouseUp = () => {
			if (drawingState.isDrawing && drawingState.currentPath) {
				finalizePath(drawingState.currentPath);
				resetDrawingState();
				updateCanvas();
			}
		};
	...
	function addLaneMarkings(path: paper.Path) {
		const centerLine = path.clone();
	...
	function smoothPath(path: paper.Path) {
		if (path.segments.length < 3) return;

		const last = path.segments[path.segments.length - 1];
		const secondLast = path.segments[path.segments.length - 2];
	...
	function finalizePath(path: paper.Path) {
	...
	const updateCanvas = throttle(() => {
		if (!roadNetwork || !simulationView) return;

		roadNetwork.view.update();

		// Only update simulation view if needed
		if (isSimulating) {
...
<main class="min-h-screen bg-gray-100 p-4">
	<div class="container mx-auto">
		<RoadEditorToolbar
			{currentTool}
			{selectedRoadType}
			on:toolChange={(e) => (currentTool = e.detail)}
			on:roadTypeChange={(e) => (selectedRoadType = e.detail)}
		/>
